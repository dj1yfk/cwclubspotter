...
openlog("rbnlive2", 'pid', 'user');


my %bl = ();
open OPTOUT, "rbn_stats_optout.txt";
while (my $line = <OPTOUT>) {
    chomp($line);
    print "Opt out: $line\n";
    $bl{$line} = 1;
}

my %conts; # remember continents of skimmers
my %calls = ();
my %spotted = ();
...
my %bm_bands = ( '160m' => 0x0001, '80m' => 0x0002, '60m' => 0x0004,
                ...
                '10m' => 0x0200, '136kHz' => 0x00400000,
                '472kHz' => 0x00200000,, '6m' => 0x00100000,
...
# de-dupe spots. we only need the information which call was spotted on which band in which hour once.
# hash with key: DXCALL-BAND-HOUR-CONTINENT, value = 1
my %dupe = ();
...
$r->subscribe('raw',
	my $callback = sub {
		...
		my $f = substr($msg, 16, 8);
		$f =~ s/[^0-9\.]//g;
        my $rbn = substr($msg, 4, 10);
		$rbn =~ s/[^A-Z0-9\/]//g;
        my $band = &freq2band($f);
        $band .= "m";
        $msg =~ /([012][0-9])[0-5][0-9]Z/;
        my $hour = $1;
        my $today = strftime("%Y-%m-%d", gmtime);

        my $cont;
        # cache continent informations (less CPU time)
        if (defined($conts{$rbn})) {
            $cont = $conts{$rbn};
        }
        else {
            $cont = (&dxcc($rbn))[3];
		...
		if (!defined($dupe{$call."-".$band."-".$hour."-".$cont."-".$today})) {
			print "New: ";
            $dupe{$call."-".$band."-".$hour."-".$cont."-".$today} = 1;
            ...
            # calculate offset of current hour
            $hour = int((str2time($today, 'GMT') - $t0)/3600) + $hour;

            my $old = substr($uncomp, $hour*4, 4);
            my $oldbm = unpack("L", $old);
            $bm |= $oldbm;
...
sub freq2band {
                ...
                if (($freq >= 135) && ($freq <= 138)) { $freq = "2200"; }
                if (($freq >= 400) && ($freq <= 550)) { $freq = "630"; }
                ...
                elsif (($freq >=60000) && ($freq <=80000)) { $freq = "6"; }
