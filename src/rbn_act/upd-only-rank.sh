...
echo "set @rank=0; create table rbn_rank_beacon select @rank:=@rank+1 as rank, hours, dxcc, callsign, beacon, wl from rbn_activity where wl=1 and beacon=1 and hours > 10 order by hours desc;" | mysql -prbnactivity -urbnactivity rbnactivity
