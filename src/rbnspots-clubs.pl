...
my @clubs = qw/CWOPS FISTS FOC HSC VHSC SHSC EHSC SKCC AGCW NAQCC BUG RCWC LIDS NRR QRPARCI CWJF/;
my %bm = ();
...
while (1) {
        ...
        elsif ($spot{freq} >= 3566.5 and $spot{freq} <= 3567.5) {   # for qrqpoint.com
            $spot{member} = 0;
            &save_spot(\%spot);
        }
